...
export default class Daemon {
  ...
  runCommand(cmd) {
    return new Promise(async resolve => {
      ...
      console.log('Confirm the cli params', blockchain.rpcUser, blockchain.rpcPass, blockchain.rpcPort);
      console.log(cmd);
      ...
      wagerrcliProcess.stderr.on('data', data => {
        console.log(`Daemon stderr: ${data}`);
        resolve(data.toString());
        // Here is where the error output goes
      });
      wagerrcliProcess.on('close', code => {
  ...
  launch(args) {
    ...
    fs.chmod(wagerrdPath, '0777', err => {
      ...
      this.wagerrdProcess.on('exit', () => errDialog.wagerrdStopped());
    });
  }
